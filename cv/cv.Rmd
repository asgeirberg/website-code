...
cv_entries %>% 
  ...
  mutate(what = glue("<span class='cv-entry-title'>{what}</span>, {where}<br style='line-height:130%;'>")) %>% 
  select(year, what) %>% 
  kable()
```
...
cv_entries %>% 
  ...
  mutate(where = if_else(!is.na(where) & !is.na(institution), glue("{where}"), where),
         ...
         when = if_else(!is.na(when), glue("<a href={when}>when</a>"), ""),
  ...
  mutate(what = glue("<span style='display:inline-block;' class='talk-title'>{what}</span>,<br style='line-height:130%;'>",
  					  "<span class='cv-entry-where'>{where}, {institution}</span>{department}{additional_info}<br>", 
                     .na = "")) %>% 
  padding_function(., "T") %>% 
  blank_year_when_repeated() %>% 
...
## Teaching {.three-split}

```{r}
cv_entries %>% 
  filter(type == 'teaching', is.na(exclude)) %>% 
  # add commas as appropriate
  mutate(where = if_else(!is.na(where) & !is.na(institution), glue("{where}"), where)) %>%
  mutate(additional_info = if_else(!is.na(additional_info), glue(", {additional_info}"), "")) %>% 
  mutate(what_translation = ifelse(!is.na(what_translation), str_c("(", what_translation, ")"), what_translation)) %>% 
  replace_na(list(what_translation = "")) %>% 
  mutate(what = glue("<span style='display:inline-block;' class='talk-title'>{what} ({when})</span><br style='line-height:130%;'>",
  					  "<span class='cv-entry-where'>{additional_info2}. </span>{department}{where}<br>", 
                     .na = "")) %>% 
  padding_function(., "C") %>% 
  blank_year_when_repeated() %>% 
  select(year, number, what) %>% 
